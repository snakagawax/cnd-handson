...
spec:
  ...
  rules:
    - host: grafana.vm18.handson.cloudnativedays.jp
      http:
        paths:
          - path: /
            pathType: Prefix
...
spec:
  ingressClassName: nginx
  rules:
    - host: prometheus.vm18.handson.cloudnativedays.jp
      http:
        paths:
          - path: /
            pathType: Prefix
...
ブラウザで  <http://prometheus.vm18.handson.cloudnativedays.jp> と <http://grafana.vm18.handson.cloudnativedays.jp> にアクセスしてみてください。
...
<http://prometheus.vm18.handson.cloudnativedays.jp/graph> にアクセスして、PromQL入力欄に `go_goroutines` と入力してみます。
...
<http://prometheus.vm18.handson.cloudnativedays.jp/alerts>
...
<http://prometheus.vm18.handson.cloudnativedays.jp/targets>
...
<http://prometheus.vm18.handson.cloudnativedays.jp/graph> を開き (またはリロードして)、PromQL入力欄に ngi と入力し、nginx のメトリクスが追加されているのを確認しましょう。
